  ...
  try {
    ...
    if (uid) {
      ...
      if (!limitCheck.allowed) {
        console.log('[sat-prep/stream] Usage limit exceeded for user:', uid);
        return new Response(
          JSON.stringify({
      ...
      console.log('[sat-prep/stream] Usage limit check passed');
    }
    ...
    const streamPromise = client.chat.completions.create({
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...(codeInterpreterParams as any), // Add code_interpreter for math accuracy
          ...
          if (!sawContent && fallbackNeeded) {
            ...
            try {
              const nonStream = await client.chat.completions.create({
                ...
                messages: baseMessages,
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                ...(codeInterpreterParams as any), // Add code_interpreter for fallback
              });
