...
export const supportKnowledgeBase: SupportKnowledgeEntry[] = [
  {
    ...
    details:
      "Lernex transforms dense manuals, curricula, and notes into cinematic micro-lessons (30-120 words) with adaptive quizzes and analytics. Primary navigation includes: /fyp (For You feed with swipeable lesson cards), /generate (convert text into AI lessons using advanced AI), /analytics (track streaks, accuracy, token usage, subject mastery), /achievements (badge roadmaps from Bronze to Mythic tiers), /playlists (organize and share lesson collections), /friends (social feed and leaderboards), /pricing (plan management), and /support (help center). Built for professionals and students to ramp up skills without burnout.",
    tags: [
      "overview",
      "lessons",
      "generate",
  ...
  },
  {
    ...
    details:
      "GENERATION LIMITS BY PLAN: Free Explorer - Standard daily limits for lesson generation at /generate (approximately 3-5 lessons per day, exact limits tracked via token usage). Limits reset at midnight in your local timezone. If you hit the limit, you'll see a friendly message with options to: wait for reset, upgrade to Plus/Premium, or continue using pre-generated FYP lessons (which don't count toward limits). Plus Momentum ($5.99/mo) - 3x higher daily limits than Free (approximately 15-20 lessons per day). Includes instant retries if generation fails. Premium Creator ($14.99/mo) - UNLIMITED generation—no daily caps or token restrictions. Generate as many lessons as you need. TRACKING USAGE: View your current usage at /analytics in the 'AI Token Usage' section. Shows input tokens, output tokens, and estimated cost. WHY LIMITS EXIST: AI generation is computationally expensive and API calls cost money. Limits ensure platform sustainability while keeping Free tier accessible. WORKAROUND: FYP lessons at /fyp are pre-generated and don't count toward your daily limits—unlimited on all plans. Only custom /generate requests count toward limits.",
  ...
  },
  {
    ...
    summary: "The /generate page converts your notes, PDFs, or study materials into structured micro-lessons with quizzes using advanced AI.",
    details:
      "Navigate to /generate to create custom lessons. Paste up to two short paragraphs of text (study notes, PDF excerpts, manuals, etc.). Select a subject from 6 domains (Math, Science, Computer Science, History, English, Languages) and choose difficulty level (intro/easy/medium/hard). Powered by state-of-the-art AI with ultra-fast inference, the system generates one 80-105 word micro-lesson with structure: definition + example + pitfall + next step. Each lesson includes 3 multiple-choice questions with max 15 word explanations per answer. Optionally add a 'next-topic hint' to guide the AI toward specific follow-up concepts. Generation typically completes in seconds. Free tier has standard limits; Plus gets 3x higher limits; Premium has unlimited generation. LaTeX math is fully supported using \\(...\\) for inline and \\[...\\] for display formulas.",
    tags: ["generate", "lesson", "ai", "quiz", "difficulty", "custom", "create", "pdf", "notes", "text", "latex", "math"],
    priority: 3,
  },
  {
